...
export default function GalleryPage() {
  ...
  const [modalTab, setModalTab] = useState<"bulk" | "manual">("bulk");
  ...
  const handleCloudinaryUploadComplete = (results: CloudinaryUploadResult[]) => {
    const newMediaItems: GalleryItem[] = results.map(r => ({
      type: "image",
      url: r.secureUrl
    }));
          ...
          <form onSubmit={handleSave} className="bg-white rounded-[28px] p-6 sm:p-8 w-full max-w-2xl shadow-2xl my-8 border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {/* Tab 1: Cloudinary Bulk Upload */}
                {modalTab === "bulk" && (
                  <div className="bg-slate-50/80 p-4 rounded-[20px] border border-slate-200/80 mb-4">
                    <CloudinaryUploader
                      folder="cechstu_gallery"
                      multiple={true}
                      maxFiles={50}
                      label="Select or Drop Multiple Images"
                      description="Directly uploads to Cloudinary and appends them to this album. Supports JPG, PNG, WEBP, GIF."
                      buttonText="Browse & Upload Images"
                      onUploadComplete={handleCloudinaryUploadComplete}
                    />
                  </div>
                )}

                {/* Tab 2: Manual URL Entry */}
                {modalTab === "manual" && (
                  <div className="bg-slate-50/80 p-4 rounded-[20px] border border-slate-200/80 mb-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        type="button" 
                        onClick={addItem} 
                        className="text-xs font-bold text-blue-600 uppercase tracking-wider flex items-center gap-1 hover:text-blue-800 bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100"
                      >
                    ...
                    <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
                      {items.map((item, index) => (
                        <div key={index} className="flex gap-2 items-center bg-white p-2.5 rounded-xl border border-slate-200 shadow-2xs">
                          ...
                          <input 
                            ...
                            className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-2 outline-none text-xs text-slate-800 focus:ring-1 focus:ring-blue-500" 
                            value={item.url} 
                      ...
                      {items.length === 0 && (
                        <p className="text-xs text-slate-400 italic py-3 text-center bg-white rounded-xl border border-dashed border-slate-200">
                          No URL items yet. Click &quot;Add URL Row&quot; or switch to Bulk Upload.
                        </p>
                      )}
                    </div>
                  </div>
