        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#020617]/50 backdrop-blur-sm p-4">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {/* Photo URL & Cloudinary Uploader */}
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest">
                  Member Photo (Upload or Manual URL)
                </label>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-start">
                  ...
                  <div className="flex-1 w-full space-y-2">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-[14px] p-3 text-[#0F172A] outline-none focus:ring-2 focus:ring-blue-500 transition-all text-xs"
                      value={formData.photoUrl}
                      onChange={(e) => setFormData({ ...formData, photoUrl: e.target.value })}
                    />

                    <CloudinaryUploader
                      folder="cechstu_members"
                      multiple={false}
                      label="Or upload member photo from device"
                      description="Uploads to Cloudinary and populates Photo URL automatically"
                      buttonText="Choose Photo"
                      onUploadComplete={(results) => {
                        if (results.length > 0) {
                          setFormData(prev => ({ ...prev, photoUrl: results[0].secureUrl }));
                        }
                      }}
                    />
                  </div>
